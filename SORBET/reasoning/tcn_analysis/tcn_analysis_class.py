...
class TCNAnalysis:
    ...
    def plot_top_marker_significance_with_direction(
        ...
        # fig.suptitle('Top Marker Significance with Direction', fontsize=32)
        # plt.tight_layout(rect=[0, 0.03, 1, 0.95])  # Removed to use constrained_layout
    ...
    #     """
    #     Extract all marker expressions per cell type and label (responders/nonresponders) across the dataset.

    #     Args:
    #         tcns_dict: Dictionary of all TCN clusters, mapping combinations to TCN objects.
    #         cell_profile_mapping: Dictionary mapping (run_id, fov_id) to cell profiles for each marker.
    #         marker_names: List of marker names, where index corresponds to marker_idx.
    #         num_markers: Expected number of markers in cell profiles.

    #     Returns:
    #         A dictionary structured as {cell_type: {label: {marker_name: all_expressions}}},
    #         where all_expressions is a concatenated list of expressions across all clusters.
    #     """
    #     all_marker_expressions = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
    ...
    #                             marker_name = marker_names[marker_idx]
    #                             all_marker_expressions[cell_type][label][marker_name].append(expression_value)

    #     return all_marker_expressions
    ...
    def plot_top_marker_sign_cell_type(marker_results, marker_names,
        ...
        def plot_violin_with_scatter(
            ...
            # # Track occupied y-positions per axis to prevent overlap
            # occupied_positions_ax = defaultdict(list)  # For individual z-scores
            # occupied_positions_ax2 = defaultdict(list)  # For combined z-scores

            # # Annotate individual extreme z-scores on primary axis
            # for i, marker in enumerate(valid_marker_list):
            #     z_scores = marker_z_scores[marker][hop]
            #     details = marker_details.get(marker, {}).get(hop, [])

            #     # Sort z-scores by absolute value and select top N
            #     sorted_indices = sorted(range(len(z_scores)), key=lambda idx: abs(z_scores[idx]), reverse=True)
            #     extreme_indices = sorted_indices[:max_extremes]

                # for idx in extreme_indices:
                #     if idx < len(details):
                #         combination_detail = details[idx]
                #     else:
                #         combination_detail = "TCN_Unknown"

                #     z_val = z_scores[idx]

                #     # Determine if annotation is eligible based on direction
                #     if direction == 'Responders' and z_val > 0:
                #         eligible = True
                #     elif direction == 'Non Responders' and z_val < 0:
                #         eligible = True
                #     else:
                #         eligible = False

                    # if eligible:
                    #     # Define offset to prevent overlap
                    #     existing_offsets = occupied_positions_ax[i + 1]
                    #     offset_increment = 0.3  # Adjust as needed
                    #     if direction == 'Responders':
                    #         # Place annotations above the point
                    #         y_position = z_val + (offset_increment * (len(existing_offsets) + 1))
                    #         vertical_alignment = 'bottom'
                    #         annotation_color = color
                    #     else:
                    #         # Place annotations below the point
                    #         y_position = z_val - (offset_increment * (len(existing_offsets) + 1))
                    #         vertical_alignment = 'top'
                    #         annotation_color = color

                    #     # Record the occupied position
                    #     occupied_positions_ax[i + 1].append(y_position)

                    #     # Add bounding box to text for better visibility
            ...
            # Optionally, annotate combined z-scores on secondary axis
            # for i, marker in enumerate(valid_marker_list):
            #     combined_z = combined_z_scores.get(marker, 0)
            #     if combined_z == 0:
            #         continue  # Skip if combined z-score is zero

            #     details = marker_details.get(marker, {}).get(hop, [])

            #     # Assuming you have details for combined z-scores, adjust as needed
            #     combination_detail = f"Combined_{marker}"  # Example label

                # # Define offset to prevent overlap
                # existing_offsets = occupied_positions_ax2[i + 1]
                # offset_increment = 0.3  # Adjust as needed
                # if combined_z > 0:
                #     y_position = combined_z + (offset_increment * (len(existing_offsets) + 1))
                #     vertical_alignment = 'bottom'
                # else:
                #     y_position = combined_z - (offset_increment * (len(existing_offsets) + 1))
                #     vertical_alignment = 'top'
                ...
                # # Record the occupied position
                # occupied_positions_ax2[i + 1].append(y_position)

                # # Add bounding box to text for better visibility
                # bbox_props = dict(boxstyle="round,pad=0.3", fc="white", ec="black", alpha=0.7)

                # # Annotate with text on secondary axis
                # ax2.text(
                #     i + 1,
                #     y_position,
                #     combination_detail,  # Label with combination detail
            ...
            # ------------------ End of Enhanced Annotation Logic ------------------
